...
{
    public partial class lanturiLungimel : Form
    {
        ...
        private void lanturiLungimel_Load(object sender, EventArgs e)
        {

        }
        private void lanturiLungimel_MouseClick(object sender, MouseEventArgs e)
        {
            n++;
            v[n] = new Button();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            int roadLength = int.Parse(textBox1.Text);
            back1 ob = new back1(n, a11, v);
            ob.back(1, n, g1, roadLength);
            DisplayMatrix(a11, n, roadLength);
        }
        private void DisplayMatrix(int[,] matrix, int size, int roadLength)
        {
            richTextBox1.AppendText("Lanturi cu lungimea " + textBox1.Text);

            for (int i = 1; i <= size; i++)
            {
                for (int j = i + 1; j <= size; j++)
                {
                    if (matrix[i, j] == roadLength)
                    {
        ...
        class back1
        {
            ...
            public void back(int K, int L, Graphics g, int roadLength)
            {
                if (K == n1 + 1)
                {
                    if (st[n1] == 1 && roadLength == n1) // verificare primul si ultimul nodului
                    {
                        tipar(K, g);
                        j++;
                    }
